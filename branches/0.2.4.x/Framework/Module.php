...
abstract class Framework_Module extends Framework_Object_Web
{
    ...
    public function set($var,$val) {
        trigger_error('Framework_Module::set() is deprecated. Please use Framework_Module::setData().', E_USER_WARNING);
    ...
    public static function &factory(Framework_Request_Common $request)
    {
        ...
        return self::tryModule($request->module, $file, $class);
    }
    ...
    public static function &tryModule($module, $file, $class)
    {
        if (!include_once($file)) {
            return PEAR::raiseError('Module file not found: ' . $file,
                                    FRAMEWORK_MODULE_ERROR_INVALID_MODULE_FILE);
        }

        if (!class_exists($class)) {
            return PEAR::raiseError('Module class not found: '. $module,
                                    FRAMEWORK_MODULE_ERROR_INVALID_MODULE_CLASS);
        }
    ...
    static public function start(Framework_Module &$module, $request)
    {
        if (!method_exists($module, $request->event) &&
            ...
            return PEAR::raiseError('Invalid event: ' . get_class($module) . '::' . $request->event. '()', FRAMEWORK_MODULE_ERROR_INVALID_EVENT);
        }
